...
import global.agenor.DefaultCoinSelector;
...
public class AgenorModuleImp implements AgenorModule {

    private static final Logger logger = LoggerFactory.getLogger(AgenorModuleImp.class);
    ...
    public AgenorModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration,ContactsStoreDao contactsStore,RateDbDao rateDb,BackupHelper backupHelper) {
    ...
    public Transaction completeTxWithCustomFee(Transaction transaction,Coin fee) throws InsufficientMoneyException{
        ...
        walletManager.completeSend(sendRequest);
        // if the fee is different to the custom fee and the tx size is lower than 1kb (1000 bytes in agenor core)
    ...
    public AgenorRate getRate(String coin) {
...
//        }
    ...
    @Override
    public List<AgenorRate> listRates() {
        return rateDb.list();
    }
    ...
    public void saveRate(AgenorRate agenorRate){
        rateDb.insertOrUpdateIfExist(agenorRate);
    }
