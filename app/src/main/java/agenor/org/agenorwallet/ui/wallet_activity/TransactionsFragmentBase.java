...
import agenor.org.agenorwallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static agenor.org.agenorwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static agenor.org.agenorwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_ZAGE_WALLET;
import static agenor.org.agenorwallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static agenor.org.agenorwallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        ...
        if (isPrivate){
            list = agenorModule.listPrivateTxes();
        }else
            list = agenorModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (agenorRate != null) {
                    localCurrency = agenorApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * agenorRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + agenorRate.getCode();
                ...
                if (data.isZcMint()){
                    ...
                    holder.title.setText(getAddressOrContact(agenorModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(agenorModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(agenorModule.getConf().getNetworkParams()).toBase58());
                }*/
        ...
        adapter.setListEventListener(new ListItemListeners<TransactionWrapper>() {
            ...
            public void onItemClickListener(TransactionWrapper data, int position) {
                ...
                bundle.putBoolean(IS_ZAGE_WALLET, isPrivate);
