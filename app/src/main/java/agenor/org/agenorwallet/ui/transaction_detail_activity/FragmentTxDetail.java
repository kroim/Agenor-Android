...
import static agenor.org.agenorwallet.ui.transaction_send_activity.custom.inputs.InputsActivity.INTENT_NO_TOTAL_AMOUNT;
...
public class FragmentTxDetail extends BaseFragment implements View.OnClickListener {
    ...
    public static final String IS_ZAGE_WALLET = "is_zpiv";
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (intent != null){
            ...
            if (intent.hasExtra(IS_DETAIL)){
                ...
                isTxDetail = true;
                if (intent.hasExtra(IS_ZAGE_WALLET)){
                    isZpivWallet = intent.getBooleanExtra(IS_ZAGE_WALLET, false);
                }
            }else {
                transactionWrapper.setTransaction(new Transaction(agenorModule.getConf().getNetworkParams(),intent.getByteArrayExtra(TX)));
    ...
    private void loadTx() {
        ...
        if (agenorRate != null) {
            txt_amount_local.setText(
                    agenorApplication.getCentralFormats().format(
                            new BigDecimal(transactionWrapper.getAmount().getValue() * agenorRate.getRate().doubleValue()).movePointLeft(8)
                    )
                            + " " + agenorRate.getCode()
            );
        } else {
        ...
        if (transactionWrapper.isStake()){
            ...
            try {
                ...
                for (TransactionInput input : transactionWrapper.getTransaction().getInputs()) {
                    ...
                    Coin unspentValue = agenorModule.getUnspentValue(unspent.getHash(), (int) unspent.getIndex());
        ...
        txt_is_tx_spent.setText(
                agenorModule.isEveryOutputSpent(
                        transactionWrapper.getTransaction(),
                        isZpivWallet ? MultiWallet.WalletType.ZAGE : MultiWallet.WalletType.AGE
            ...
            if (transactionWrapper.getOutputLabels()!=null && transactionWrapper.getOutputLabels().containsKey(transactionOutput.getIndex())){
                ...
                if (addressLabel !=null) {
                    if (addressLabel.getName() != null) {
                        ...
                        label = transactionOutput.getScriptPubKey().getToAddress(agenorModule.getConf().getNetworkParams(),true).toBase58();
                }else {
                    label = transactionOutput.getScriptPubKey().getToAddress(agenorModule.getConf().getNetworkParams(),true).toBase58();
                }
            }else {
                ...
                if (script.isPayToScriptHash() || script.isSentToRawPubKey() || script.isSentToAddress()) {
                    label = script.getToAddress(agenorModule.getConf().getNetworkParams(), true).toBase58();
                }else {
    ...
    private void setupOutputs(List<OutputUtil> list) {
        ...
        ListItemListeners<OutputUtil> listItemListener = new ListItemListeners<OutputUtil>() {
            ...
            public void onLongItemClickListener(OutputUtil data, int position) {
                if (agenorModule.chechAddress(data.getLabel())) {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.txt_inputs){
            try {
                Set<InputWrapper> set = agenorModule.convertFrom(transactionWrapper.getTransaction().getInputs());
