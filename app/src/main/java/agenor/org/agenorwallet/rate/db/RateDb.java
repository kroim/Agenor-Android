package agenor.org.agenorwallet.rate.db;
...
import agenor.org.agenorwallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<AgenorRate> implements RateDbDao<AgenorRate> {
    ...
    @Override
    protected ContentValues buildContent(AgenorRate obj) {
    ...
    @Override
    protected AgenorRate buildFrom(Cursor cursor) {
        ...
        return new AgenorRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(AgenorRate agenorRate) {
        if (getRate(agenorRate.getCode())==null){
            insert(agenorRate);
        }else {
            updateByKey(KEY_COIN,agenorRate.getCode(),agenorRate);
        }
    }
}
